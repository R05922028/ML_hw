...
for i in range(iteration):
    ...
    for j in range(len(train[9])-10):
        ...
        w_grad = w_grad - 2.0*(ideal - bias - (weights*input_data).sum() - (weights_2*input_data*input_data).sum())*input_data - 2 * lam * weights 
        w_grad_2 = w_grad_2 - 2.0*(ideal - bias - (weights*input_data).sum() - (weights_2*input_data*input_data).sum())*input_data*input_data - 2 * lam * weights_2
        loss = loss +math.pow((ideal - bias - (weights*input_data).sum()-(weights_2*input_data*input_data).sum()),2) + lam * np.power(weights,2).sum() + lam * np.power(weights_2,2).sum()
    lr_b = lr_b + b_grad ** 2
    lr_w = lr_w + w_grad ** 2
...
fout = open('submit_all_regularization_0001.csv','w')
fout.write('id,value\n')
