...
prediction_data = "prediction_bow.csv"
...
X_test_bow = np.zeros((len(X_test), len(dic)))
...
for i in range(len(X_test)):
  for j in range(len(X_test[i])):
    if X_test[i][j] in dic:
      X_test_bow[i][int(dic[X_test[i][j]])-1] += 1
    else:
      X_test_bow[i][len(dic)-1] += 1
...
#model = Sequential()
#model.add(Dense(1, activation='sigmoid', input_dim = len(dic)))
